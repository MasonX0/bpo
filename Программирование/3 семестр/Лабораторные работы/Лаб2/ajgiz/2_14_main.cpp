...
int main() {
    ...
    // РАСПРЕДЕЛЯЕМ слова по файлам
    for (int i = 0; i < cnt; i++) {
        // В UTF-8 каждая русская буква = 2 байта
        // Поэтому делим длину в байтах на 2, чтобы получить количество букв
        int char_count = list[i].length() / 2;

        if (char_count % 2 == 0) {  // Если четное количество букв
            chet_output << list[i] << endl;
        } else {                    // Если нечетное количество букв
